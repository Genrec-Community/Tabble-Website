...
Task ID: 6
Agent: main (Super Z)
Task: Integrate the FeatureShowcase component (shadcn) into the Features page with warm-theme adaptation; remove public pricing (request-access/contact instead); migrate domain to tabble.in with contact@tabble.in; wire Resend transactional email for contact + waitlist forms.

Work Log:
- Confirmed codebase already shadcn + Tailwind 4 + TS; default component path is src/components/ui (@/components/ui alias) — no setup needed. All required shadcn primitives (tabs/accordion/badge/card/button/input/label) + Radix deps already installed; only `resend` added.
- globals.css: added accordion-down/up @theme inline animations + keyframes (Radix height var) per the component's CSS requirements — accordions now animate (previously snapped).
- Created src/components/ui/feature-showcase.tsx: copy of the provided component, theme-corrected from the dark demo to Tabble warm tokens (semantic vars resolve to cream/ink/tangerine; Fraunces display H2; tangerine-soft eyebrow pill; tangerine-deep primary CTA; espresso/cream active tab states; wrap-safe bottom tab pill). Extended TabMedia with optional `content` ReactNode (live mockups instead of images) + optional ctaPrimary/ctaSecondary.
- Features page: new TourSection right under the header — FeatureShowcase with 3 tabs (Guest menu = PhoneDemo in espresso phone frame / Kitchen = KitchenDisplay / Insights = InsightsMock), 4-step journey accordion (scan → kitchen → pay → numbers), stats chips, Request-access + Talk-to-us CTAs, 580px panel. kds-mock.tsx gained non-breaking `forceTwoCols` prop (2-col tickets on mobile inside fixed-height frames). Deep-dive ViewSections re-visualized with vision-verified photos (diners / kitchen team / interior) so the mocks aren't duplicated; section tone rhythm re-flowed (cream header → white tour → cream guests → deep kitchen → white owner → cream more).
- Pricing removed: NAV_ITEMS + footer link -Pricing; PRICING content export deleted; the 4 money FAQs folded into FAQS (tag Pricing) with answers de-referenced from the pricing page; "What does it cost?" rewritten to onboarding-call framing; /pricing route now permanentRedirect → /request-access (308 verified in browser); sitemap -/pricing; SoftwareApplication JSON-LD dropped its ₹ offers; home + features bottom CTAs "See pricing" → "Talk to us" (contact). Founding-cohort/founding-pricing copy retained (concept, not a price list).
- Domain + email: SITE_URL → https://tabble.in (canonicals, sitemap, robots, OG, JSON-LD all derive), SITE_EMAIL + BRAND.email → contact@tabble.in, OG image pill → tabble.in. Public-facing email everywhere is contact@tabble.in.
- Resend: resend@6.29.0 installed; .env.local (gitignored via existing .env* rule) holds RESEND_API_KEY/CONTACT_EMAIL/EMAIL_FROM; src/lib/email.ts = server-only helper (lazy client, never throws, branded cream/espresso HTML templates, reply-to sender); /api/contact + /api/waitlist send best-effort inbox notifications (DB stays source of truth, emailSent flag in response). Resend testing restriction discovered: unverified sender only delivers to account owner — CONTACT_EMAIL temporarily set to the owner inbox so mail works today; documented the one-line switch to contact@tabble.in after DNS-verifying tabble.in in Resend.
- Fixed pre-existing lint/tsc errors (not caused by this task, surfaced by bun run lint): order-relay FlightTimer rewritten to derive display time from phase (no setState off-fly), steps-bento PayLoop reduced-motion setState moved into rAF (hydration-safe), kds-mock rotation `status: "new" as const` (slice() broke contextual typing).
- Verification: lint + tsc clean (project files); all 11 routes 200; /pricing 308 → /request-access (browser-confirmed); canonical = https://tabble.in/features; sitemap 0 pricing refs; contact form + waitlist POSTs → ok:true + emailSent:true (Resend delivered); FAQ JSON-LD 12 Questions; no /pricing links or Offer schema anywhere; agent-browser: tour tabs click-through (KDS 578px, Insights, accordion expand), tablist 309px single row on 375px, KDS clears the floating pill (4px gap, mobile+desktop, pb-14 fix after VLM flagged overlap), zero horizontal overflow 1440/375; VLM: desktop tour 9/10 wow, fixed kitchen tab 8/10 zero collisions; home FAQ ticker renders the 12-question set; console + dev.log clean.

Stage Summary:
- Features page now leads with an interactive tabbed product tour (FeatureShowcase) — component lives at src/components/ui/feature-showcase.tsx, fully theme-adapted and reusable.
- Public pricing is gone: /pricing permanently redirects to request-access; every conversion path is Request access or Talk to us; rates are shared on the onboarding call (founding-cohort story intact).
